# Sample Weekly Review

Week: 2026-W23
Owner: Example maintainer

## Results

| Metric | This week | Last week | Change |
| --- | ---: | ---: | ---: |
| Impressions | 12000 | 9000 | +3000 |
| Link clicks | 240 | 210 | +30 |
| Free completions | 36 | 42 | -6 |
| Paid offer clicks | 14 | 10 | +4 |
| Revenue | 600 | 400 | +200 |

## Wins

- Short video reach improved.
- Paid CTA clicks increased.

## Losses

- Free completion rate dropped.

## Bottleneck

The weakest step this week was free completion.

Evidence: Link clicks increased, but completions decreased.

## Decision Rule Applied

- Clearest drop-off: free completion rate.
- Reviewable in under 30 minutes: rewrite one form intro paragraph.
- Avoided: price changes, payment settings, production secrets, and private customer data.
- Number to watch next week: free completions from the same link click volume.

## Decision

Next week we will reduce free form friction and clarify the result promise.

## Not Doing

- Redesigning the full site.
- Changing prices.
- Changing payment settings.

## Codex Task

Review the free entry copy and suggest one smaller, clearer version that reduces hesitation without changing the offer, price, payment settings, or customer data handling.

